...
func (d *Dir) Create(ctx context.Context, req *fuse.CreateRequest, resp *fuse.CreateResponse) (node fusefs.Node, handle fusefs.Handle, err error) {
	fmt.Println("mount/dir/Create函数创建新文件,name=%q", req.Name)
	body := "{\"createTimestamp\": \"$currentTimeStamp\", \"entityId\": \"001\", \"dataValue\": 2, \"dataRecNum\": 1,\"receipts\": [{\"keyId\": \"lwq|test.mp4|v1\", \"receiptValue\": 1, \"version\": \"v1\", \"userName\": \"lwq\", \"operationType\": \"create\", \"dataType\": \"\", \"serviceType\": \"school\", \"fileName\": \"test.mp4\", \"fileSize\": 100, \"fileHash\": \"asdfghjkl\", \"uri\": \"/abc\", \"parentKeyId\": \"000\", \"attachmentFileUris\": [\"/aaa\", \"/bbb\"], \"attachmentTotalHash\": \"zxcvbnm\"}]}"
	_, err = http.Post("192.168.195.197:9000/storeReceipt", "application/json;charset=utf-8", strings.NewReader(string(body)))
...
func (d *Dir) Mkdir(ctx context.Context, req *fuse.MkdirRequest) (node fusefs.Node, err error) {
	fmt.Println("mount/dir/Mkdir函数创建新文件夹name=%q", req.Name)
	defer log.Trace(d, "name=%q", req.Name)("node=%+v, err=%v", &node, &err)
...
func (d *Dir) Remove(ctx context.Context, req *fuse.RemoveRequest) (err error) {
	fmt.Println("mount/dir/Remove函数移除=%q",req.Name)
...
func (d *Dir) Rename(ctx context.Context, req *fuse.RenameRequest, newDir fusefs.Node) (err error) {
	fmt.Println("mount/dir/Rename函数重命名 文件=%q",req.OldName)
...
func (d *Dir) Fsync(ctx context.Context, req *fuse.FsyncRequest) (err error) {
	fmt.Println("mount/dir/Fsync函数同步文件夹")
